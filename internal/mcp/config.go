...
// runtime.Tool values, namespaced per PLAN.md section 8 ("server github +
// tool search -> github.search"). One long-lived process is kept per unique
// server config and shared across every namespace that points at it; a
// crashed process is respawned lazily on the next call.
package mcp
